...
# Using uv (faster)
uv pip install git+https://github.com/HazyResearch/flash-fft-conv.git#subdirectory=csrc/flashfftconv
...
# Install CUDA kernels (for 1D FlashFFTConv)
...
For **N-D FFT convolutions only** (FlashFFTConv2D/3D), no CUDA compilation needed:
```bash
git clone https://github.com/HazyResearch/flash-fft-conv.git
cd flash-fft-conv
...
# Or install from requirements.txt first
...
### N-D FFT Convolutions (2D/3D) for ConvSSM

We also provide 2D and 3D FFT convolutions for ConvSSM (Convolutional State Space Models):

```python
from flashfftconv import FlashFFTConv2D, FlashFFTConv3D

# 2D convolution (for video frames)
fftconv2d = FlashFFTConv2D(height=64, width=64, dtype=torch.bfloat16)
x = torch.randn(4, 128, 64, 64)   # (B, C, H, W)
k = torch.randn(128, 3, 3)        # (C, Kh, Kw) - small 3x3 kernel
y = fftconv2d(x, k)               # Output: (4, 128, 64, 64)

# 3D convolution (for volumetric data)
fftconv3d = FlashFFTConv3D(depth=16, height=64, width=64)
x = torch.randn(4, 128, 16, 64, 64)  # (B, C, D, H, W)
k = torch.randn(128, 3, 3, 3)        # (C, Kd, Kh, Kw) - small 3x3x3 kernel
y = fftconv3d(x, k)                  # Output: (4, 128, 16, 64, 64)
```

**ConvSSM usage** - the recurrence `h_t = A ★ h_{t-1} + B ★ x_t`:
```python
fftconv = FlashFFTConv2D(H, W)
A_kernel = torch.randn(C, 3, 3) * 0.01  # State transition kernel
B_kernel = torch.randn(C, 3, 3) * 0.01  # Input kernel

# Single timestep
h_new = fftconv(h_prev, A_kernel) + fftconv(x, B_kernel)

# Multiple timesteps (receptive field expands!)
for t in range(T):
    h = fftconv(h, A_kernel) + fftconv(x[t], B_kernel)
    # t=1: 3x3 receptive field
    # t=2: 5x5 receptive field (A★A)
    # t=T: (2T+1)x(2T+1) receptive field
```

**Why FFT for ConvSSM?** The parallel scan over time requires computing A^T (kernel convolved with itself T times). In pixel space, this kernel grows to size O(T×K). In FFT space, it stays constant size, enabling O(log T) parallel depth.

Run the N-D tests:
```bash
pytest tests/test_conv_nd.py -v
