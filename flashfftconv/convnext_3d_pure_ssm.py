"""ConvNeXt with Pure 3D ConvSSM - SSM replaces depthwise conv entirely.

Key difference from convnext_3d_ssm.py:
- In the original, each block has: FFTDepthwiseConv3D (7×7×1) + ParallelConvSSM3D (K×K×T)
- In this pure version, each block has: ParallelConvSSM3D only (K×K×T)

The ConvSSM IS the spatial mixing operation, not an addition to depthwise conv.

Architecture:
1. Input (B, H, W, 3) -> repeat T times -> (B, T, H, W, 3)
2. 3D stem conv: (B, T, H/4, W/4, C)
3. Pure 3D ConvSSM blocks (no separate depthwise conv)
...
# =============================================================================
# 3D Parallel ConvSSM (Pure - replaces depthwise conv)
# =============================================================================

class PureConvSSM3D(nn.Module):
    """Pure 3D ConvSSM that replaces depthwise convolution entirely.

    Uses 3D FFT for efficient convolution and associative scan for
    O(log T) parallel recurrence computation.

    Key difference: This module does the job of BOTH the depthwise conv
    and the SSM mixing - it's not an addition on top of a depthwise conv.
    ...
    """
    dim: int
    kernel_size: int = 7  # Spatial (H, W) - same as original depthwise conv
...
# =============================================================================
# Pure ConvNeXt 3D Block (SSM replaces depthwise conv)
# =============================================================================

class PureConvNeXt3DSSMBlock(nn.Module):
    """Pure ConvNeXt block where ConvSSM REPLACES depthwise conv.

    Unlike ConvNeXt3DSSMBlock which has FFTDepthwiseConv3D + ParallelConvSSM3D,
    this block uses PureConvSSM3D as the ONLY spatial mixing operation.

    Architecture:
    1. PureConvSSM3D (handles ALL spatial/temporal mixing - replaces dwconv)
    ...
    def __call__(self, x: jnp.ndarray, train: bool = True) -> jnp.ndarray:
        ...
        # 1. Pure 3D ConvSSM (replaces BOTH depthwise conv and additional SSM)
        # This is the KEY DIFFERENCE - no separate depthwise conv!
        x = PureConvSSM3D(
            dim=C,
            kernel_size=self.kernel_size,  # 7 spatial
            kernel_size_t=self.kernel_size_t,  # 5 temporal
            dtype=self.dtype,
            name='pure_convssm3d'
        )(x)
        ...
        # 3. Pointwise MLP (applied per-position)
...
# =============================================================================
# Full Pure 3D ConvNeXt-SSM Model
# =============================================================================

class PureConvNeXt3DSSM(nn.Module):
    """Pure ConvNeXt with 3D ConvSSM replacing depthwise conv.

    Unlike ConvNeXt3DSSM which has FFTDepthwiseConv3D + ParallelConvSSM3D
    per block, this model uses ONLY PureConvSSM3D as the spatial mixing.

    Input image is repeated T times to create pseudo-video (B, T, H, W, 3),
    then processed through pure 3D ConvSSM blocks.

    Architecture per block:
    - PureConvSSM3D: kernel_size × kernel_size × kernel_size_t (ONLY operation)
    - No separate depthwise conv
    ...
    Attributes:
        ...
        kernel_size: Spatial kernel size (H, W) for ConvSSM
        kernel_size_t: Temporal kernel size (T) for ConvSSM
    ...
    kernel_size: int = 7  # Spatial (H, W) for ConvSSM
    kernel_size_t: int = 5  # Temporal (T) for ConvSSM
...
def pure_convnext_3d_ssm_tiny(
    num_classes: int = 1000,
    T: int = 8,
    kernel_size: int = 7,  # Spatial kernel
    kernel_size_t: int = 5,  # Temporal kernel
    **kwargs
) -> PureConvNeXt3DSSM:
    """Pure ConvNeXt-3D-SSM-Tiny.

    Architecture per block:
    - PureConvSSM3D: kernel_size × kernel_size × kernel_size_t (ONLY operation)
    - No separate depthwise conv
    """
    return PureConvNeXt3DSSM(
...
def pure_convnext_3d_ssm_small(
    num_classes: int = 1000,
    T: int = 8,
    kernel_size: int = 7,  # Spatial kernel
    kernel_size_t: int = 5,  # Temporal kernel
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("TEST: Pure ConvNeXt-3D-SSM (ConvSSM replaces depthwise conv)")
    print("=" * 70)

    key = random.PRNGKey(0)

    # Test with T=4, kernel_size=7 (spatial), kernel_size_t=5 (temporal)
    print("\nT=4, kernel_size=7 (spatial), kernel_size_t=5 (temporal):")
    model = pure_convnext_3d_ssm_tiny(num_classes=10, T=4, kernel_size=7, kernel_size_t=5)
    ...
    model_t8 = pure_convnext_3d_ssm_tiny(num_classes=10, T=8, kernel_size=7, kernel_size_t=5)
